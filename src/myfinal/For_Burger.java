 ...
 * @author ShariQ
 */
public class For_Burger {
    private int remainingAmount;
    private int totalAmount;
    private static int quantityOfBURGER=0;
    Scanner scan=new Scanner(System.in);
    int amountOfBURGER = 100;
    ...
    public For_Burger(int[][] TT,int IS,int FS,int[] possibleAmount){
        this.TT=TT;
        this.IS=IS;
        this.FS=FS;
    ...
    public boolean transaction(int credit,int st){
        ...
        if(st==FS){
            remainingAmount=credit-amountOfBURGER;
            
            if(remainingAmount>=amountOfBURGER){
                
                while(credit-amountOfBURGER >= 100){
            ...
            if(st!=FS){
                while(st!=FS){
                    System.out.println("Not enough amount to purchase Burger");
