 ...
 * @author ShariQ
 */
public class For_IceCream {
    private int remainingAmount;
    private int totalAmount;
    private static int quantityOfIC=0;
    ...
    public For_IceCream(int[][] TT,int IS,int FS,int[] possibleAmount){
        this.TT=TT;
        this.IS=IS;
        this.FS=FS;
    ...
    public boolean transaction(int credit,int st){
        ...
        if(st==FS){
            remainingAmount=credit-amountOfIC;
            
            if(remainingAmount>=amountOfIC){
                
                while(credit-amountOfIC >= 50){
            ...
            if(st!=FS){
                while(st!=FS){
                    System.out.println("Not enough amount to purchase Ice-Cream");
